...
namespace Domain.Services.Product.Queries.Handlers
{
    public class GetProductsQueryHandler : IRequestHandler<GetProductsQuery, List<GetProductResponse>>
    {
        ...
        public GetProductsQueryHandler(IProductReadOnlyRepository productReadOnlyRepository, IMapper mapper)
        {
        ...
        public async Task<List<GetProductResponse>> Handle(GetProductsQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var products = _mapper.Map<List<ProductDto>>(await _productReadOnlyRepository.GetProducts());
